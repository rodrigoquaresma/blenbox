...
function CalcStats(form) {
  ...
  yq=(Cell_A*Cell_D-Cell_B*Cell_C)/(Cell_B*Cell_C+Cell_A*Cell_D);
  form.yq.value=Fmt(yq)
  ets = (Cell_A - Sav_A) / (Cell_A + Cell_B + Cell_C - Sav_A);
  form.ets.value=Fmt(ets)
  ...
  form.Low_A.value=Fmt(Ex_A);
  form.Low_B.value=Fmt(Ex_B);
  form.Low_C.value=Fmt(Ex_C);
  form.Low_D.value=Fmt(Ex_D);
  od=(Ex_A/Ex_B)/(Ex_C/Ex_D);
  form.od_lo.value=Fmt(od)
  rr=(Ex_A/Cell_r1)/(Ex_C/Cell_r2);
  form.rr_lo.value=Fmt(rr)
  kp=2*(Ex_A*Ex_D-Ex_B*Ex_C)/((Ex_B+Ex_A)*(Ex_B+Ex_D)+(Ex_A+Ex_C)*(Ex_D+Ex_C));
  form.kp_lo.value=Fmt(kp)
  fc=(Ex_A+Ex_D)/(Ex_B+Ex_A+Ex_D+Ex_C);
  form.fc_lo.value=Fmt(fc);
  form.mcr_hi.value=Fmt(1-fc)
  sn=Ex_A/Cell_c1;
  form.sn_lo.value=Fmt(sn)
  sp=Ex_D/Cell_c2;
  form.sp_lo.value=Fmt(sp)
  plr=sn/(1-sp);
  form.plr_lo.value=Fmt(plr)
  nlr=(1-sn)/sp;
  form.nlr_hi.value=Fmt(nlr)
  dor=(sn/(1-sn))/((1-sp)/sp);
  form.dor_lo.value=Fmt(dor)
  eor=(sn/(1-sn))/(sp/(1-sp));
  form.eor_lo.value=Fmt(eor)
  yj=sn+sp-1;
  form.yj_lo.value=Fmt(yj)
  nnd=1/yj;
  form.nnd_hi.value=Fmt(nnd)
  nnm=1/(1-fc);
  form.nnm_lo.value=Fmt(nnm)
  pp=Ex_A/Cell_r1;
  form.pp_lo.value=Fmt(pp)
  np=Ex_D/Cell_r2;
  form.np_lo.value=Fmt(np)
  dplo=Ex_A/Cell_r1-Ex_C/Cell_r2;
  form.dp_lo.value=Fmt(dplo)
  arr=-dplo;
  form.arr_hi.value=Fmt(arr)
  rrr=arr/(Ex_C/Cell_r2);
  form.rrr_hi.value=Fmt(rrr)
  nmi = 1 - ( xlx(Ex_B+Ex_A) + xlx(Ex_D+Ex_C) - xlx(Ex_B) - xlx(Ex_A) - xlx(Ex_D) - xlx(Ex_C) ) / ( xlx(t) - xlx(Cell_c2) - xlx(Cell_c1) );
  form.nmi_lo.value=Fmt(nmi)
  csny=csq(Ex_B,Sav_B,0)+csq(Ex_A,Sav_A,0)+csq(Ex_D,Sav_D,0)+csq(Ex_C,Sav_C,0)
  cc=Sqrt(csny/(csny+t));
  form.cc_lo.value=Fmt(cc)
  ca=cc*Sqrt(2);
  form.ca_lo.value=Fmt(ca)
  rtet=Cos(Pi/(1+Sqrt(od)));
  form.rtet_lo.value = Fmt(rtet)
  cp=(Ex_A*Ex_D-Ex_B*Ex_C)/Sqrt(Cell_r1*Cell_r2*Cell_c2*Cell_c1);
  form.cp_lo.value=Fmt(cp)
  yq=(Ex_A*Ex_D-Ex_B*Ex_C)/(Ex_B*Ex_C+Ex_A*Ex_D);
  form.yq_lo.value=Fmt(yq)
  ets = (Ex_A - Sav_A) / (Ex_A + Ex_B + Ex_C - Sav_A);
  form.ets_lo.value=Fmt(ets)
  EntR = - ( fEnt(Cell_r1/t) + fEnt(Cell_r2/t) );
  form.EntR_lo.value=Fmt(EntR)
  EntC = - ( fEnt(Cell_c1/t) + fEnt(Cell_c2/t) );
  form.EntC_lo.value=Fmt(EntC)
  EntRC = - ( fEnt(Ex_A/t) + fEnt(Ex_B/t) + fEnt(Ex_C/t) + fEnt(Ex_D/t) );
  form.EntRC_lo.value=Fmt(EntRC)
  EntIB = EntR + EntC - EntRC;
  form.EntIB_hi.value=Fmt(EntIB)
  EntIA = EntRC - EntR;
  form.EntIA_lo.value=Fmt(EntIA)
  EntIC = EntRC - EntC;
  form.EntIC_lo.value=Fmt(EntIC)
  EntSim = EntIB / ( EntIA + EntIB + EntIC );
  form.EntSim_hi.value=Fmt(EntSim)
  EntDif = 1 - EntSim;
  form.EntDif_lo.value=Fmt(EntDif)
  del=HiSlop
  Ex_B=Cell_B-HiSlop;
  Ex_A=Cell_A+HiSlop;
  Ex_D=Cell_D+HiSlop;
  ...
  form.High_A.value=Fmt(Ex_A);
  form.High_B.value=Fmt(Ex_B);
  form.High_C.value=Fmt(Ex_C);
  form.High_D.value=Fmt(Ex_D);
  od=(Ex_A/Ex_B)/(Ex_C/Ex_D);
  form.od_hi.value=Fmt(od)
  rr=(Ex_A/Cell_r1)/(Ex_C/Cell_r2);
  form.rr_hi.value=Fmt(rr)
  kp=2*(Ex_A*Ex_D-Ex_B*Ex_C)/((Ex_B+Ex_A)*(Ex_B+Ex_D)+(Ex_A+Ex_C)*(Ex_D+Ex_C));
  form.kp_hi.value=Fmt(kp)
  fc=(Ex_A+Ex_D)/(Ex_B+Ex_A+Ex_D+Ex_C);
  form.fc_hi.value=Fmt(fc);
  form.mcr_lo.value=Fmt(1-fc)
  sn=Ex_A/Cell_c1;
  form.sn_hi.value=Fmt(sn)
  sp=Ex_D/Cell_c2;
  form.sp_hi.value=Fmt(sp)
  plr=sn/(1-sp);
  form.plr_hi.value=Fmt(plr)
  nlr=(1-sn)/sp;
  form.nlr_lo.value=Fmt(nlr)
  dor=(sn/(1-sn))/((1-sp)/sp);
  form.dor_hi.value=Fmt(dor)
  eor=(sn/(1-sn))/(sp/(1-sp));
  form.eor_hi.value=Fmt(eor)
  yj=sn+sp-1;
  form.yj_hi.value=Fmt(yj)
  nnd=1/yj;
  form.nnd_lo.value=Fmt(nnd)
  nnm=1/(1-fc);
  form.nnm_hi.value=Fmt(nnm)
  pp=Ex_A/Cell_r1;
  form.pp_hi.value=Fmt(pp)
  np=Ex_D/Cell_r2;
  form.np_hi.value=Fmt(np)
  dphi=Ex_A/Cell_r1-Ex_C/Cell_r2;
  form.dp_hi.value=Fmt(dphi)
  arr=-dphi;
  form.arr_lo.value=Fmt(arr)
  rrr=arr/(Ex_C/Cell_r2);
  form.rrr_lo.value=Fmt(rrr)
  ...
  cc=Sqrt(csny/(csny+t));
  form.cc_hi.value=Fmt(cc)
  ca=cc*Sqrt(2);
  form.ca_hi.value=Fmt(ca)
  rtet=Cos(Pi/(1+Sqrt(od)));
  form.rtet_hi.value = Fmt(rtet)
  cp=(Ex_A*Ex_D-Ex_B*Ex_C)/Sqrt(Cell_r1*Cell_r2*Cell_c2*Cell_c1);
  form.cp_hi.value=Fmt(cp)
  yq=(Ex_A*Ex_D-Ex_B*Ex_C)/(Ex_B*Ex_C+Ex_A*Ex_D);
  form.yq_hi.value=Fmt(yq)
  ets = (Ex_A - Sav_A) / (Ex_A + Ex_B + Ex_C - Sav_A);
  form.ets_hi.value=Fmt(ets)
  EntR = - ( fEnt(Cell_r1/t) + fEnt(Cell_r2/t) );
  form.EntR_hi.value=Fmt(EntR)
  EntC = - ( fEnt(Cell_c1/t) + fEnt(Cell_c2/t) );
  form.EntC_hi.value=Fmt(EntC)
  EntRC = - ( fEnt(Ex_A/t) + fEnt(Ex_B/t) + fEnt(Ex_C/t) + fEnt(Ex_D/t) );
  form.EntRC_hi.value=Fmt(EntRC)
  EntIB = EntR + EntC - EntRC;
  form.EntIB_lo.value=Fmt(EntIB)
  EntIA = EntRC - EntR;
  form.EntIA_hi.value=Fmt(EntIA)
  EntIC = EntRC - EntC;
  form.EntIC_hi.value=Fmt(EntIC)
  EntSim = EntIB / ( EntIA + EntIB + EntIC );
  form.EntSim_lo.value=Fmt(EntSim)
  EntDif = 1 - EntSim;
  form.EntDif_hi.value=Fmt(EntDif)
  if(dp==0) {
    form.nn.value="Infinite"
  } else {
    form.nn.value=Fmt(Math.abs(1/dp))
  }
  form.nn_lo.value="Unknown";
  form.nn_hi.value="Unknown"
  if(dplo<0 & dphi<0) {
    form.nn_lo.value=Fmt(-1/dplo);
    form.nn_hi.value=Fmt(-1/dphi)
  }
  if(dplo<0 & dphi==0) {
    form.nn_Lo.value=Fmt(-1/dplo);
    form.nn_hi.value="Infinite"
  }
  if(dplo<0 & dphi>0) {
    form.nn_lo.value=Fmt(1/Math.max(Math.abs(dplo),Math.abs(dphi)));
    form.nn_hi.value="Infinite"
  }
  if(dplo==0 & dphi>0) {
    form.nn_lo.value=Fmt(1/dphi);
    form.nn_hi.value="Infinite"
  }
  if(dplo>0 & dphi>0) {
    form.nn_lo.value=Fmt(1/dphi);
    form.nn_hi.value=Fmt(1/dplo)
  }
}
